...
export function RankChart({ data }: RankChartProps) {
  ...
  const dataWithKeys = data.map((d, index) => ({ ...d, uniqueKey: index }));

  return (
    <div className="space-y-2">
      <ResponsiveContainer width="100%" height={200}>
        <LineChart data={dataWithKeys} margin={{ top: 10, right: 10, left: -20, bottom: 0 }}>
          ...
          <Tooltip
            ...
            formatter={(value: number) => [`Rang ${value}`, '']}
...
export function PointsChart({ data }: PointsChartProps) {
  ...
  return (
    <ResponsiveContainer width="100%" height={160}>
      <BarChart data={dataWithKeys} margin={{ top: 10, right: 10, left: -20, bottom: 0 }}>
        <CartesianGrid strokeDasharray="3 3" stroke="rgba(255,255,255,0.08)" vertical={false} />
        <XAxis
          dataKey="label"
          tick={{ fill: '#999999', fontSize: 10 }}
